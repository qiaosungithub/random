...
def prepare_batch_data(batch, config, batch_size=None):
    ...
    if config.model.use_aug_label:
        ...
        augment_label = augment_label.reshape(
            (local_device_count, -1) + augment_label.shape[1:]
        )
    ...
    if split == "train":
        if dataset_cfg.root == "MNIST":
            ...
            ds = datasets.CIFAR10(
                ...
                download=True,
                transform=transforms.Compose(
                    ([transforms.RandomHorizontalFlip(p=0.5)] if dataset_cfg.use_flip else []) + [
            ...
            raise NotImplementedError("Unknown dataset root: {}".format(dataset_cfg.root))
